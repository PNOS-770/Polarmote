...
- `asmote_create_session(config_json)`
- `asmote_destroy_session(session_id)`
- `asmote_enqueue_transfer(session_id, task_json)`
- `asmote_cancel_task(session_id, task_id)`
- `asmote_query_progress(session_id, task_id)`
- `asmote_poll_events(session_id)`
- `asmote_free_c_string(ptr)`
...
- Windows: `target/<profile>/asmote_native_core.dll`
- Linux: `target/<profile>/libasmote_native_core.so`
- macOS: `target/<profile>/libasmote_native_core.dylib`
...
- Build script: `native/asmote_native_core/scripts/build_android_libs.ps1` (Windows)
- Build script: `native/asmote_native_core/scripts/build_android_libs.sh` (macOS/Linux)
- Output directory: `android/app/build/generated/rustJniLibs/<abi>/libasmote_native_core.so`
...
- Build only specific ABIs (greatly reduces Android debug build time):
  - Environment variable: `ASMOTE_ANDROID_ABIS=x86_64`
  - Or Gradle property: `-PasmoteRustAbis=x86_64`
...
4. Perl is available for vendored OpenSSL build.
   ...
   - If `Locale::Maketext::Simple` / `ExtUtils::MakeMaker` are missing, scripts auto-load shim from
     `native/asmote_native_core/scripts/perl_lib`.
...
6. Windows path encoding:
   - If detected rust sysroot contains non-ASCII characters, script auto-switches to
     project-local Rustup/Cargo homes under `native/asmote_native_core/.rustup-local` and
     `native/asmote_native_core/.cargo-local`.

Skip Rust Android build temporarily:

```powershell
set ASMOTE_SKIP_RUST_ANDROID=1
```
...
- Script: `native/asmote_native_core/scripts/build_ios_lib.sh`
- Linked output: `ios/Flutter/asmote_native/<CONFIGURATION>/<SDK_NAME>/libasmote_native_core.a`

The script builds Rust static libraries for iOS targets and links with:

- `-force_load $(PROJECT_DIR)/Flutter/asmote_native/$(CONFIGURATION)/$(SDK_NAME)/libasmote_native_core.a`
...
- Force universal simulator lib (slower):
  - `ASMOTE_IOS_SIM_UNIVERSAL=1`

## Desktop Incremental Build

Desktop packaging now uses script-based incremental Rust build:

- Windows script: `native/asmote_native_core/scripts/build_desktop_libs.ps1`
- Linux/macOS script: `native/asmote_native_core/scripts/build_desktop_libs.sh`
...
- iOS symbol missing errors:
  ...
  - Confirm output static library exists under `ios/Flutter/asmote_native/...`.
