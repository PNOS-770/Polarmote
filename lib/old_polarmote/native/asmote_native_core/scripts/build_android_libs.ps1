...
function Ensure-MakeAvailable {
  ...
  foreach ($candidate in $gmakeCandidates) {
    ...
    $shimDir = Join-Path $env:TEMP "asmote-native-tool-shims"
...
Set-Item -Path Env:ASMOTE_BUILD_ID -Value $buildId
Set-Item -Path Env:ASMOTE_BUILD_PROFILE -Value $Profile
...
foreach ($target in $targets) {
  ...
  $libPath = Join-Path $CrateDir "target/$triple/$Profile/libasmote_native_core.so"
  ...
  $outputLibPath = Join-Path $abiDir "libasmote_native_core.so"
