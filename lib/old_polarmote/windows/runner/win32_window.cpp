...
static void AllowCrossIntegrityDragDrop(HWND hwnd) {
  ...
  // WM_COPYGLOBALDATA (0x0049) -- required by OLE drag-drop (IDropTarget)
  // which is the mechanism used by super_drag_and_drop / super_native_extensions.
  ChangeWindowMessageFilterEx(hwnd, 0x0049, MSGFLT_ADD, nullptr);
}
...
// App window minimum size in logical pixels.
...
bool Win32Window::Create(const std::wstring& title,
  ...
  UpdateTheme(window);
  AllowCrossIntegrityDragDrop(window);
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    ...
    case WM_SYSKEYDOWN: {
      if (wparam == VK_INSERT) {
        const bool wasPreviouslyDown = (lparam & (1 << 30)) != 0;
