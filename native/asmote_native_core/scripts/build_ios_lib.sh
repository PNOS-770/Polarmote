...
OUTPUT_DIR="$IOS_DIR/Flutter/asmote_native/$CONFIGURATION_NAME/$SDK_NAME"
OUTPUT_LIB="$OUTPUT_DIR/libasmote_native_core.a"
mkdir -p "$OUTPUT_DIR"
...
build_target_if_needed() {
  ...
  lib_path="$CRATE_DIR/target/$target/$PROFILE/libasmote_native_core.a"
  needs_build=1
  if [ -f "$lib_path" ]; then
...
if [ "$SDK_NAME" = "iphoneos" ]; then
  ...
  cp "$CRATE_DIR/target/aarch64-apple-ios/$PROFILE/libasmote_native_core.a" "$OUTPUT_LIB"
else
  sim_targets=""
  if [ "${ASMOTE_IOS_SIM_UNIVERSAL:-0}" = "1" ]; then
  ...
  lib_arm64="$CRATE_DIR/target/aarch64-apple-ios-sim/$PROFILE/libasmote_native_core.a"
  lib_x86_64="$CRATE_DIR/target/x86_64-apple-ios/$PROFILE/libasmote_native_core.a"
