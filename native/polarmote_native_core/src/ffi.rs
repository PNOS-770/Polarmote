...
fn build_info_payload() -> String {
    json!({
        ...
        "version": option_env!("Polarmote_ENGINE_VERSION").unwrap_or(env!("CARGO_PKG_VERSION")),
        "build_id": option_env!("Polarmote_BUILD_ID").unwrap_or("unknown"),
        "profile": option_env!("Polarmote_BUILD_PROFILE").unwrap_or("unknown"),
    })
    .to_string()
}

#[no_mangle]
pub extern "C" fn Polarmote_create_session(config_json: *const c_char) -> u64 {
...
#[no_mangle]
pub extern "C" fn Polarmote_destroy_session(session_id: u64) -> i32 {
...
#[no_mangle]
pub extern "C" fn Polarmote_enqueue_transfer(session_id: u64, task_json: *const c_char) -> i32 {
...
#[no_mangle]
pub extern "C" fn Polarmote_cancel_task(session_id: u64, task_id: *const c_char) -> i32 {
...
#[no_mangle]
pub extern "C" fn Polarmote_query_progress(session_id: u64, task_id: *const c_char) -> *mut c_char {
...
#[no_mangle]
pub extern "C" fn Polarmote_poll_events(session_id: u64) -> *mut c_char {
...
#[no_mangle]
pub extern "C" fn Polarmote_build_info() -> *mut c_char {
    into_c_string(build_info_payload())
}

#[no_mangle]
pub extern "C" fn Polarmote_runtime_create(config_json: *const c_char) -> u64 {
...
#[no_mangle]
pub extern "C" fn Polarmote_runtime_destroy(runtime_id: u64) -> i32 {
...
#[no_mangle]
pub extern "C" fn Polarmote_session_open(runtime_id: u64, config_json: *const c_char) -> u64 {
    let Ok(config_text) = ptr_to_string(config_json) else {
        return 0;
    };
    let Ok(config) = serde_json::from_str::<SessionConfig>(&config_text) else {
...
#[no_mangle]
pub extern "C" fn Polarmote_session_close(runtime_id: u64, session_id: u64) -> i32 {
    let Ok(runtime) = RuntimeRegistry::global().runtime(runtime_id) else {
        return -1;
    };
...
#[no_mangle]
pub extern "C" fn Polarmote_session_submit_graph(
    runtime_id: u64,
    session_id: u64,
...
#[no_mangle]
pub extern "C" fn Polarmote_session_cancel_graph(
    runtime_id: u64,
    session_id: u64,
    graph_id: u64,
) -> i32 {
...
#[no_mangle]
pub extern "C" fn Polarmote_session_poll_events_cursor(
    runtime_id: u64,
    session_id: u64,
    cursor: u64,
    limit: u32,
...
#[no_mangle]
pub extern "C" fn Polarmote_session_query_metrics(runtime_id: u64, session_id: u64) -> *mut c_char {
    let result = (|| -> CoreResult<String> {
        let runtime = RuntimeRegistry::global()
            .runtime(runtime_id)
            .map_err(|error| CoreError::Internal(error.to_string()))?;
...
#[no_mangle]
pub extern "C" fn Polarmote_free_c_string(ptr: *mut c_char) {
    if ptr.is_null() {
        return;
    }
    unsafe {
