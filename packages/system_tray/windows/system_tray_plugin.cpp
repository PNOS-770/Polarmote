...
LRESULT CALLBACK HostCloseSubclassProc(HWND hwnd, UINT msg,
                                       WPARAM wp, LPARAM lp,
...
class SystemTrayPlugin : public flutter::Plugin, public SystemTray::Delegate {
 ...
 protected:
  ...
  virtual void OnMenuItemSelected(int32_t id);
 ...
 protected:
  ...
  // HWND of the host (main) window, used for WM_CLOSE interception.
  HWND host_window_hwnd_ = nullptr;
...
SystemTrayPlugin::~SystemTrayPlugin() {
  ...
  if (host_window_hwnd_) {
    RemoveWindowSubclass(host_window_hwnd_, HostCloseSubclassProc, 0);
  }
}
...
void SystemTrayPlugin::initSystemTray(
  ...
  do {
    ...
    host_window_hwnd_ = window;
    SetWindowSubclass(window, HostCloseSubclassProc, 0, 0);
...
void SystemTrayPlugin::OnMenuItemSelected(int32_t id) {
...
void SystemTrayPlugin::initAppWindow(
  ...
  do {
    ...
    if (!app_window_->initAppWindow(window, view->GetNativeWindow())) {
