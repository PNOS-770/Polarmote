...
void SystemTray::ShowPopupMenu() {
  ...
  TrackPopupMenu(context_menu_, TPM_LEFTBUTTON, pt.x, pt.y, 0, tray_window_,
                 nullptr);
...
LRESULT SystemTray::TrayMessageHandler(HWND window,
  ...
  if (message == WM_COMMAND) {
    ...
    if (menu_id >= kFirstMenuId && delegate_) {
      delegate_->OnMenuItemSelected(static_cast<int32_t>(menu_id - kFirstMenuId));
      return 0;
    }
    return DefWindowProc(window, message, wparam, lparam);
  }
  ...
  if (message == tray_notify_callback_message_) {
    ...
    UINT notifyMsg = LOWORD(lparam);
    POINT pt = {GET_X_LPARAM(wparam), GET_Y_LPARAM(wparam)};
    (void)OnTrayIconCallback(id, notifyMsg, pt);
    return 0;
  }
  return DefWindowProc(window, message, wparam, lparam);
}
