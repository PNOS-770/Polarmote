...
class RenderTerminal extends RenderBox with RelayoutWhenSystemFontsChangeMixin {
  ...
  void _updateCursorBlink() {
    if (_terminal.cursorBlinkMode && _focusNode.hasFocus) {
      _startCursorBlink();
    } else {
      _stopCursorBlink();
    }
  ...
  void _onControllerUpdate() {
    // Controller updates (selection/highlight/pointer flags) do not affect
    // geometry, only painting.
    markNeedsPaint();
  }
  ...
  bool get _shouldShowCursor {
    if (_terminal.cursorBlinkMode) {
      return (_terminal.cursorVisibleMode || _alwaysShowCursor || _isComposingText) && _cursorBlinkVisible;
    }
